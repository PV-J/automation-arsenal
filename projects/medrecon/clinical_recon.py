...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="MedRecon — Patient Data Reconciliation & Clinical Anomaly Detection"
    )
    parser.add_argument(
        "--ehr",
        help="Path to EHR demographics CSV file"
    )
    parser.add_argument(
        "--lab",
        help="Path to Lab demographics CSV file"
    )
    parser.add_argument(
        "--pharmacy",
        help="Path to Pharmacy data CSV file (optional)"
    )
    parser.add_argument(
        "--threshold",
        type=float,
        default=0.75,
        help="Patient matching confidence threshold (0.0-1.0)"
    )
    parser.add_argument(
        "--full-recon",
        action="store_true",
        help="Run full reconciliation including lab trends and medications"
    )
    parser.add_argument(
        "--report", "-r",
    ...
    # Initialize engine
    engine = ClinicalReconciliationEngine(match_threshold=args.threshold)

    if args.demo or (not args.ehr and not args.lab):
        # Demo mode with synthetic data
        print("=" * 60)
        print("MEDRECON — CLINICAL RECONCILIATION ENGINE DEMO")
        print("=" * 60)

        # Create temp directory for demo files
        tmp_dir = Path(tempfile.gettempdir()) / "medrecon_demo"
        tmp_dir.mkdir(exist_ok=True)

        # Create synthetic EHR data
        ...
        # Create synthetic Lab data (with one intentional mismatch)
        lab_data = pd.DataFrame({
            'patient_id': ['PT001', 'PT002', 'PT003'],
            'mrn': ['', '', ''],
            'first_name': ['John', 'Jane', 'Robert'],
            'last_name': ['Smith', 'Doe', 'Johnsen'],  # Note: typo in Johnson
        ...
        # Save demo files
        ehr_path = tmp_dir / "ehr_demographics.csv"
        lab_path = tmp_dir / "lab_demographics.csv"
        ehr_data.to_csv(ehr_path, index=False)
        lab_data.to_csv(lab_path, index=False)

        # Ingest
        ehr_patients = engine.ingest_patient_demographics(ehr_path, 'EHR_System')
        lab_patients = engine.ingest_patient_demographics(lab_path, 'Lab_System')
        ...
        print(f"  Partial matches (review):   {len(results['partial_matches'])}")
        print(f"  Unmatched in EHR only:      {len(results['unmatched_source_a'])}")
        print(f"  Unmatched in Lab only:      {len(results['unmatched_source_b'])}")

        if results['partial_matches']:
            print(f"\n  ⚠️  Partial match found (likely data entry error):")
            for match in results['partial_matches']:
                print(f"    EHR: {match['patient_a'].last_name_hash[:8]}... vs Lab: {match['patient_b'].last_name_hash[:8]}...")
                print(f"    Confidence: {match['confidence']:.2f}")

        # Demo lab analysis — direct demonstration
        print(f"\n{'=' * 60}")
        print("LAB ANALYSIS (Simulated)")
        print(f"{'=' * 60}")

        # Create lab results and classify severity directly
        critical_lab = LabResult(
            patient_id_hash="demo_patient",
            test_name='POTASSIUM',
        ...
        results_to_check = [
            ("POTASSIUM", critical_lab),
            ("GLUCOSE", normal_lab),
            ("CREATININE", borderline_lab),
        ]

        print(f"\nTests analyzed: {len(results_to_check)}")
        
        critical_count = 0
        for test_name, lab_result in results_to_check:
            severity = lab_result.classify_severity()
            icon = {"critical": "🔴", "abnormal": "🟠", "borderline": "🟡", "normal": "🟢"}.get(severity.value, "⚪")
            print(f"  {icon} {test_name}: {lab_result.value} {lab_result.unit} "
                  f"(Ref: {lab_result.reference_range_low}-{lab_result.reference_range_high}) "
                  f"— {severity.value.upper()}")
            
            if severity == SeverityLevel.CRITICAL:
                critical_count += 1
                action = engine._recommend_action(test_name, lab_result.value, 0)
        ...
        if args.report:
            ...
            engine.generate_reconciliation_report(report_path)
        ...
        # Real file mode
        if not args.ehr or not args.lab:
            print("Error: Both --ehr and --lab are required for real data mode.")
            print("Use --demo for demonstration with synthetic data.")
            exit(1)

        ehr_path = Path(args.ehr)
        lab_path = Path(args.lab)

        if not ehr_path.exists():
            print(f"Error: EHR file not found: {args.ehr}")
            exit(1)
        if not lab_path.exists():
            print(f"Error: Lab file not found: {args.lab}")
            exit(1)

        print(f"Ingesting EHR data: {ehr_path.name}")
        ehr_patients = engine.ingest_patient_demographics(ehr_path, 'EHR_System')

        print(f"Ingesting Lab data: {lab_path.name}")
        lab_patients = engine.ingest_patient_demographics(lab_path, 'Lab_System')
        ...
        print(f"  Unmatched EHR: {len(results['unmatched_source_a'])}")
        print(f"  Unmatched Lab: {len(results['unmatched_source_b'])}")

        if args.report:
            engine.generate_reconciliation_report(Path(args.report))
