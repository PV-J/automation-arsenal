...
#Suppress sqlite3 datetime deprecation warning (Python 3.12+)
import warnings
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="WatchTower — Multi-endpoint Health Monitor"
    )
    parser.add_argument(
        "--config", "-c",
        default="config.yaml",
    ...
    parser.add_argument(
        ...
        type=int,
        default=24,
        help="Hours to analyze for trend report"
    )
    ...
    if not config_path.exists():
        ...
        print("Using default endpoints (localhost for demo)")
        endpoints = [
            EndpointConfig(
        ...
        for ep in config.get("endpoints", []):
            endpoints.append(EndpointConfig(
                ...
                check_ssl=ep.get("check_ssl", True),
                ssl_expiry_warning_days=ep.get("ssl_expiry_warning_days", 30),
                response_time_threshold_ms=ep.get("response_time_threshold_ms", 2000),
            ))
    ...
    async def run():
        ...
        if args.report:
            ...
            print("\n" + "=" * 50)
            print("WATCHTOWER — TREND REPORT")
            print("=" * 50)
            for endpoint in endpoints:
                degradation = monitor.trend_analyzer.detect_degradation(
                    endpoint.url, window_hours=args.hours
                )
                print(f"\n{endpoint.url}")
                if degradation.get("degradation_detected"):
                    print(f"  ⚠️  {degradation['alert_message']}")
                else:
                    print(f"  ✓  No degradation detected")
                if "baseline_avg_ms" in degradation:
                    print(f"  Baseline: {degradation['baseline_avg_ms']:.0f}ms")
                    print(f"  Recent:   {degradation['recent_avg_ms']:.0f}ms")
        else:
            # Run health check
            ...
            for result in results:
                ...
                print(f"{status} {result.url}: {result.status_code or 'N/A'} ({result.response_time_ms:.0f}ms){error}")

            # SSL warnings
            for result in results:
                if result.ssl_expiry_date:
                    days_left = (result.ssl_expiry_date - datetime.now()).days
                    if days_left < 30:
                        print(f"\n⚠️  SSL WARNING: {result.url}")
                        print(f"   Certificate expires in {days_left} days")
                        print(f"   Issuer: {result.ssl_issuer}")
